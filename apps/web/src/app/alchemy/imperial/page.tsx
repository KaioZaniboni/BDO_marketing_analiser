...
export default function ImperialAlchemyPage() {
    ...
    const masteryBonus = getImperialBonus(settings.alchemyMastery);

    return (
        <div className="flex flex-col gap-6">
            <div>
                <h1 className="text-2xl font-bold text-primary flex items-center gap-2">
                    <Crown size={24} className="text-purple-400" /> Alquimia Imperial
                </h1>
                <p className="text-sm text-secondary mt-1">Cálculo de rentabilidade focado nas entregas ao NPC vs Venda Direta.</p>
            </div>
            <div className="card p-4 flex gap-6 items-center text-sm">
                <div><span className="text-xs text-secondary">Maestria Alquimia:</span><span className="ml-2 font-mono font-bold text-purple-400">{settings.alchemyMastery}</span></div>
                <div><span className="text-xs text-secondary">Bônus de Lucro Imperial:</span><span className="ml-2 font-mono font-bold text-profit">+{masteryBonus.toFixed(1)}%</span></div>
            </div>
                            ...
                            rankings.map(r => (
                                <tr key={r.id} className="border-t border-border hover:bg-bg-hover/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className="bg-purple-400/10 text-purple-400 px-2 py-1 rounded text-xs font-bold tracking-wide">
